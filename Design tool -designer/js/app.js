...
var mainControls = function (blueprint3d) {
  ...
  function saveDesign() {
    ...
    console.log(data);
    a.download = "designer.blueprint3d";
...
function addBlueprintListeners(blueprint3d) {
  ...
  function addGUIFolder(o) {
    //    	console.log(o.type, o.item);
    if (currentFolder) {
      gui.removeFolder(currentFolder.name);
    }
    if (o.type == BP3DJS.EVENT_CORNER_2D_CLICKED) {
      currentFolder = CornerProperties(o.item, gui); //getCornerPropertiesFolder(gui, o.item);
...
function datGUI(three, floorplanner) {
  ...
  carbonPropsFolder = getCarbonSheetPropertiesFolder(
    gui,
...
$(document).ready(function () {
  ...
  // console.log("loadUrl is =", loadUrl);
  fetch(loadUrl, { method: "GET" })
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      blueprint3d.model.loadSerialized(loadData);
    });
