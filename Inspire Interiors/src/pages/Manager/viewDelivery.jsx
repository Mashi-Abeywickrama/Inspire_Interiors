        ...
        <>
            <div className="order-container w-100 rounded-3 mb-4 me-5 p-3">
                <div className="d-flex flex-row gap-4">
                    <Link to="/manager/delivery"><p className="text-dark fs-5 fw-bold Cabin-text">Delivery</p></Link>
                ...
                <div className="col-12 d-flex flex-column">
                    <div className="d-flex flex-row justify-content-between">
                        <p className="text-dark fs-6 fw-bold text-decoration-underline Cabin-text">Delivery Details - #{orderData.ref_no}</p>
                            ...
                            <div className="d-flex flex-column">
                                ...
                                <div className="d-flex flex-row justify-content-between">
                                    ...
                                    <button
                                        ...
                                        style={{
                                            height: "1.5rem",
                                            background: (orderData.status === 'Completed') ? "#007F00" : (orderData.status === 'Delivered') ? "#bfe5fd" : "#E6E6E6",
                                            color: (orderData.status === 'Completed') ? "#fff" : (orderData.status === 'Delivered') ? "#023047" : "#979797"
                                        }}
                                        disabled={orderData.status === 'Completed' || orderData.status === 'Delivered'}
                                    >
                                        <Icon.CircleFill
                                            size={7}
                                            className="mx-1"
                                            color={(orderData.status === 'Completed') ? '#fff' : (orderData.status === 'Delivered') ? '#023047' : '#979797'}
                                        />
                                        {orderData.status === 'Completed' ? 'Completed' : (orderData.status === 'Delivered' ? 'Delivered' : 'Pending')}
                                    </button>

                                </div>
                                <div className="d-flex flex-row justify-content-between">
                                    <p className="fs-6 fw-semibold px-5 Cabin-text">Completion:</p>
                                    <button
                                        onClick={() => {
                                            if (orderData.status === 'Delivered') {
                                                // Change the status to 'Completed' if it was 'Delivered'
                                        ...
                                        style={{
                                            height: "1.5rem",
                                            background: (orderData.status === 'Completed') ? "#007F00" : (orderData.status === 'New' || orderData.status === 'Delivered' || orderData.status === 'Cancelled' || orderData.status === 'Confirmed' || orderData.status === 'Prepared') ? "#E6E6E6" : "#bfe5fd",
                                            color: (orderData.status === 'Completed') ? "#fff" : (orderData.status === 'New' || orderData.status === 'Cancelled' || orderData.status === 'Confirmed' || orderData.status === 'Delivered' || orderData.status === 'Prepared') ? "#979797" : "#023047"
                                        }}
                                        disabled={orderData.status === 'Shipped' || orderData.status === 'Completed' || orderData.status === 'New' || orderData.status === 'Prepared'}
                                    >
                                        <Icon.CircleFill
                                            size={7}
                                            className="mx-1"
                                            color={(orderData.status === 'Completed') ? '#fff' : (orderData.status === 'New' || orderData.status === 'Delivered' || orderData.status === 'Cancelled' || orderData.status === 'Confirmed' || orderData.status === 'Prepared') ? '#979797' : '#023047'}
                                        />
                                        {orderData.status === 'Delivered' ? 'Mark as Completed' : (orderData.status === 'Completed' ? 'Completed' : 'Pending')}
                                    </button>
