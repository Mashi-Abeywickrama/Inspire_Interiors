...
const TypeView = () => {
    ...
    async function fetchAndStoreProductData(type) {
        try {
            const response = await axiosInstance.get(`category-by-type/${type}`);
                                ...
                                <Link to='/customer/marketplace'>
                                    <Carousel.Caption className='d-flex justify-content-startpx-2' style={{ position: "absolute", top: 0, left: 0 }}>
                                        <div className=" align-items-center fs-3 ms-3" >
                                            <FontAwesomeIcon className='me-2 fs-3' icon={faArrowLeft} size="lg" />{type}
                                        </div>
